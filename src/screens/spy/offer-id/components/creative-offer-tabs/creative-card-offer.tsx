import { getExtension } from "@/screens/spy/creatives/helper/get-extension";
...
export const CreativeCardOffer = ({ data, ...props }: { data: ISpyCreative } & CardProps) => {
  ...
  return (
    <Card className="card" shadow="none" radius="lg" {...props}>
      <CardBody className="flex flex-col p-0 h-full aspect-[9/16]">
        <Tooltip content={`Total de criativos: ${data.totalCreatives}`} placement="top">
          <Button
            title="transcription"
            isIconOnly
            radius="full"
...
export const CreativeCardOfferLoading = () => {
  return (
