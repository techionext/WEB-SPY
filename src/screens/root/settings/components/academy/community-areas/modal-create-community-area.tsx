...
import { useCreateCommunityAreaMutation } from "@/services/community/settings/community-area.service";
import { usePathname } from "next/navigation";

export const ModalCreateCommunityArea = () => {
  const { isOpen, onOpen, onOpenChange } = useDisclosure();
  const [createCommunityArea, { isLoading }] = useCreateCommunityAreaMutation();
  const {
    handleSubmit,
    control,
    reset,
  ...
  const onSubmit = (formData: SchemaCommunityAreaOutput) => {
    createCommunityArea(formData).unwrap();
    handleClose();
  };
  ...
  const isCommunityScrenn = usePathname().includes("/community");
          ...
          {(onClose) => (
            <form onSubmit={handleSubmit(onSubmit)}>
              <ModalHeader className="flex flex-col gap-1">
                <span className="text-xl font-semibold">Nova Área da Comunidade</span>
              </ModalHeader>
