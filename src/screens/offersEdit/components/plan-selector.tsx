...
interface PlanSelectorProps {
  plans: IPlan[];
  selectedPlans: Array<{ id: string; url: string; cpa: string }>;
  onPlansChange: (plans: Array<{ id: string; url: string; cpa: string }>) => void;
  error?: any;
}
...
export const PlanSelector = ({ plans, selectedPlans, onPlansChange, error }: PlanSelectorProps) => {
  ...
  const handleTogglePlan = (planId: string, isSelected: boolean) => {
    if (isSelected) {
      const newPlan = { id: planId, url: "", cpa: formatCurrency(0, false, currency) };
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3">
        {plans.map((plan, index) => {
          ...
          return (
            <Card key={plan.id} shadow="none" className="bg-default-100/50">
